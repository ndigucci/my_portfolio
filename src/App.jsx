...
import Terminal from './components/Terminal'; // Importing the new Smart Terminal
...
// --- COMPONENT: LIVE CV SHEET (FULLY RESTORED) ---
...
function App() {
  ...
  // --- PROJECTS DATA (UPDATED WITH IMAGES) ---
  const projects = [
    {
      id: 1,
  ...
  return (
    <div className={`min-h-screen bg-[#0a0a0a] text-gray-200 font-sans selection:bg-cyan-500 selection:text-black ${(activeModal || showCV || showCrypto || activeBlog) ? 'overflow-hidden max-h-screen' : ''}`}>
      ...
      {/* --- SKILLS SECTION (RESTORED GRID LAYOUT) --- */}
      <RevealOnScroll>
      ...
      {/* --- PROJECTS SECTION (UPDATED WITH IMAGE PREVIEWS) --- */}
      <RevealOnScroll>
      ...
      {/* --- CERTIFICATES SECTION (RESTORED SWIPE LAYOUT) --- */}
      <RevealOnScroll>
