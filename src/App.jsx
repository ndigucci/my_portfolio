    ...
    if (e.key === 'Enter') {
      ...
      switch (cmd) {
        case 'help':
          ...
          break;
        case 'whoami':
          response = 'User: Guest | Role: Recruiter/Visitor | Access: Restricted';
          break;
        case 'skills':
          response = '加载中... Python, Kali Linux, Wireshark, Burp Suite, Metasploit';
          break;
        case 'contact':
          response = 'Email: irumvabonheur@icloud.com | Status: Open to work';
          break;
        case 'clear':
          setHistory([]);
          setInput('');
...
// --- COMPONENT: RESEARCH MODAL (For Research Project) ---
  ...
  <div className="bg-zinc-900 w-full h-full p-8 rounded-lg border border-gray-700 overflow-y-auto relative">
    ...
    <div className="space-y-6 text-gray-300 leading-relaxed">
      ...
      <div className="pt-6 border-t border-gray-800">
        <a href="#" className="inline-flex items-center text-cyan-400 hover:text-cyan-300 transition">
          View Full Publication <span className="ml-2">→</span>
        </a>
      </div>
    </div>
  </div>
...
function App() {
  ...
  // Projects Data with Types
  const projects = [
    {
      id: 1,
      title: "AI in Pharmacy",
      subtitle: "Machine Learning for Medication Safety",
      type: "research", // Triggers Research Modal
      desc: "Research exploring artificial intelligence in medication safety and clinical decision support.",
      abstract: "This study investigates the efficacy of Machine Learning algorithms in detecting potential drug-drug interactions (DDIs) within hospital databases. By analyzing over 10,000 patient records...",
      findings: [
        "Identified 15% more potential interactions than standard rule-based systems.",
        "Reduced alert fatigue by categorizing risk levels.",
        "Implemented using Python (Scikit-Learn) and Pandas."
      ],
    ...
    },
    {
      ...
      type: "terminal", // Triggers Terminal Modal
    ...
    },
    {
      ...
      abstract: "A comprehensive review of digital tools used in modern pharmaceutical research, focusing on data integrity and citation management systems.",
      findings: [
        "Comparative analysis of Mendeley vs. Zotero in clinical workflows.",
        "Automated citation formatting using Python scripts."
      ],
  ...
  return (
    <div className={`min-h-screen bg-[#0a0a0a] text-gray-200 font-sans selection:bg-cyan-500 selection:text-black ${activeModal ? 'overflow-hidden max-h-screen' : ''}`}>
            ...
            <div className="hidden md:block">
              <div className="ml-10 flex items-baseline space-x-8">
                {['Home', 'About', 'Skills', 'Projects', 'Contact'].map((item) => (
                  <button key={item} onClick={() => scrollToSection(item.toLowerCase())} className="text-sm font-medium hover:text-cyan-400 transition-colors duration-300 relative group">
        ...
        <div className="max-w-6xl mx-auto z-10 w-full flex flex-col md:flex-row items-center justify-between gap-12">
          ...
          <div className="text-center md:text-left flex-1">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tight">
              ...
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-cyan-400 to-teal-400">BONHEUR</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
              <button onClick={() => scrollToSection('projects')} className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-lg transition-all shadow-[0_0_20px_rgba(8,145,178,0.3)] hover:shadow-[0_0_30px_rgba(8,145,178,0.5)]">
                View Projects
              </button>
              <button onClick={() => scrollToSection('contact')} className="px-8 py-3 border border-gray-700 hover:border-gray-500 text-gray-300 hover:text-white font-medium rounded-lg transition-all">
                Contact Me
              </button>
            </div>
          </div>
            ...
            <div className="p-6 bg-[#111] border border-gray-800 rounded-xl hover:border-cyan-500/50 transition-colors duration-300 group">
              ...
              <div className="flex flex-wrap gap-2">
                {['Python', 'Linux (Kali)', 'AI Tools', 'Cybersecurity', 'Git'].map(skill => (
                  <span key={skill} className="text-xs font-mono px-2 py-1 bg-gray-800 text-cyan-400 rounded border border-gray-700">{skill}</span>
                ))}
              </div>
            </div>
            <div className="p-6 bg-[#111] border border-gray-800 rounded-xl hover:border-green-500/50 transition-colors duration-300 group">
              ...
              <div className="flex flex-wrap gap-2">
                {['Pharmacy', 'Med Safety', 'Research', 'Patient Care'].map(skill => (
                  <span key={skill} className="text-xs font-mono px-2 py-1 bg-gray-800 text-green-400 rounded border border-gray-700">{skill}</span>
                ))}
              </div>
            </div>
      ...
      {/* --- PROJECTS SECTION (UPDATED FOR INTERACTIVITY) --- */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <div 
                ...
                className="group relative bg-[#111] rounded-2xl p-6 border border-gray-800 hover:border-gray-600 transition-all duration-300 hover:-translate-y-2 cursor-pointer"
              >
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-6">
                    <div className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center">
                      <span className="text-xl">{project.type === 'terminal' ? '📟' : '📄'}</span>
                    </div>
                  ...
                  <div className="mt-4 text-xs font-bold text-cyan-500 uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity">
                    {project.type === 'terminal' ? '> Launch Terminal' : '> View Research'}
                  </div>
                </div>
              </div>
            ))}
